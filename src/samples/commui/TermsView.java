    ...
    protected void doLogin()
    {
        synchronized (this) {
            ...
            if (!pendingLogin && !community.isLoggedIn()) {
                ...
                itemList.setItem("Email", community.username);
                itemList.setItem("password", community.password);
    ...
    public boolean handleEvent(Event e) {
    	...
    	if (e.getType() == Event.ITEM_DESELECTED) {
    		if (e.getValue().equals(Community.YES)) {
	        	...
	        	community.switchToView( Community.WELCOME+" "+community.username);
	        }
    		return true;
    	}
    	return false;
     ...
     public boolean commandCompleted(String cmd, String errorMessage, int errorCode, ItemList results) {
        ...
        // Success results
	    	...
	    	if (cmd.equals( "createUser")) {
	    		//doLogin();
                    System.out.println("hi hi hi hih i");
